...
static void drawPixelsXRGB8888(uint8_t* dst, uint8_t* src, ptrdiff_t dst_stride, ptrdiff_t src_stride,
                              int width, int height) {
    auto fun = [](uint8_t* d, uint8_t* s, int n) {
        while (n -- > 0) {
            d[0] = s[1];
            d[1] = s[2];
            d[2] = s[3];
...
static void drawRGB(uint8_t* dst,  uint8_t* src, ptrdiff_t dst_stride, ptrdiff_t src_stride, int src_format, int dataLength, int offset, int scanLength,
